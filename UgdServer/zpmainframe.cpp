...
void  ZPMainFrame::on_evt_SocketError(QObject * senderSock ,const QString &erro)
{
    //return ;
    QDateTime dtm = QDateTime::currentDateTime();
    QString msg = dtm.toString("yyyy-MM-dd HH:mm:ss.zzz") + " " + QString("SockError %1 with code %2")
            .arg((quint64)senderSock).arg(erro);
    qDebug() << msg;
    return;
    int nrows = m_pMsgModelNetwork->rowCount();
    m_pMsgModelNetwork->insertRow(0,new QStandardItem(msg));
...
//The socket error message
void  ZPMainFrame::on_evt_SocketError_Cluster(QObject * senderSock ,const QString &erro)
{
    //return ;
    QDateTime dtm = QDateTime::currentDateTime();
    QString msg = dtm.toString("yyyy-MM-dd HH:mm:ss.zzz") + " " + QString("SockError %1 with code %2")
