...
namespace ExampleServer{
    ...
    private:
        ...
        //获取产品信息文件名
        static QString getProductInfoFileName(const QString &product);
        //获取产品保存路径
        static QString getProductSavePath(const QString &product);

	protected:
        static QString S_sSaveRootPath; //保存根目录
        static QString S_sProductFile;

        QMutex m_mutex_equipID;
		//log in
        bool m_bLoggedIn;
        QScopedPointer<QFile> m_uploadFileHandle;
        BD_REQUEST_UPLOAD_FILE_S m_uploadFileInfo;
        uint32_t m_receivedBytes;   //已接收字节数
        QScopedPointer<QFile> m_downloadFileHandle;
