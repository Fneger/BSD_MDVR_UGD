...
void TestServer::newCon(QAsioTcpsocket *socket)
{
    ...
    connect(socket,&QAsioTcpsocket::bytesWritten,this,&TestServer::some_data_sended);
...
void TestServer::some_data_sended(qint64 wsended)
{
    qDebug() << QTime::currentTime().toString("hh:mm:ss.zzz") << "some_data_sended" << wsended;
}
